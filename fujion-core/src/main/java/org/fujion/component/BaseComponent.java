...
public abstract class BaseComponent implements IElementIdentifier {
     ...
     *
     * @param onHandler Value as &lt;eventName&gt; \n &lt;script&gt;
     */
    @PropertySetter("#on")
    private void setOnHandler(String onHandler) {
        String[] pcs = onHandler.split("\n", 2);
        IScriptLanguage lang = ScriptRegistry.getInstance().get("groovy");
        IParsedScript script = lang.parse(pcs[1]);
        addEventListener(pcs[0], (event) -> {
            Map<String, Object> variables = new HashMap<>();
            variables.put(lang.getSelf(), this);
            variables.put("event", event);
            variables.put("controller", this.findAttribute("_controller"));
    ...
    public void wireController(Object controller) {
        ...
        setAttribute("_controller", controller);
